...
export const useAuth = () => {
  ...
  return {
    ...
    isOrganization: user?.role === 'organization' || user?.role === 'organization_admin' || user?.role === 'organization_member',
    isOrganizationAdmin: user?.role === 'organization' || user?.role === 'organization_admin',
    isOrganizationMember: user?.role === 'organization_member',
    isSeeker: user?.role === 'seeker',
    isVerificationOfficer: user?.role === 'verification_officer',
    isModerator: user?.role === 'moderator',
    isTrustUser: ['admin', 'super_admin', 'verification_officer', 'moderator'].includes(user?.role || ''),
