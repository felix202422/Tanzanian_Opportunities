  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">Privileged access oversight — who holds elevated roles</p>
      </div>
      ...
      <Card className="p-4 bg-slate-50 border">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-tdop-navy">{users.length} users with privileged access</p>
            <p className="text-xs text-gray-500">Only ADMIN, SUPER_ADMIN, MODERATOR, and VERIFICATION_OFFICER roles shown.</p>
          </div>
        </div>
      </Card>
      ...
      <Card className="overflow-hidden">
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-gray-50 border-b">
                ...
                <th className="text-left px-4 py-3 text-xs font-semibold text-gray-600 uppercase">Verified</th>
                ...
                <tr key={user.userId} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    {user.enabled ? (
                      ...
                      <span className="flex items-center gap-1 text-xs text-red-600"><XCircle className="w-3 h-3" /> Disabled</span>
                    )}
                  </td>
                  <td className="px-4 py-3">
                    {user.verified ? (
        ...
        {users.length === 0 && (
          <div className="p-8 text-center text-gray-500 text-sm">No privileged users found.</div>
        )}
      </Card>
    </div>
  );
