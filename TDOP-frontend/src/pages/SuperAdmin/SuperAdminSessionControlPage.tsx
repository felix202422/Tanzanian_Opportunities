  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">User session overview and access management</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Enabled Users', value: data.enabledUsers },
          { label: 'Disabled Users', value: data.disabledUsers },
          { label: 'Admin Sessions (Enabled)', value: data.adminSessions },
        ].map((item) => (
      ...
      <Card className="p-5">
        <h3 className="text-sm font-semibold text-tdop-navy mb-3 flex items-center gap-2">
          <Shield className="w-4 h-4" /> Privileged Access Summary
        </h3>
        <div className="space-y-2">
          <div className="flex justify-between text-sm">
            <span className="text-gray-500">Active privileged users (ADMIN, SA, MOD, VO)</span>
            <span className="font-medium text-tdop-navy">{data.privilegedActive}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-gray-500">Disabled users</span>
            <span className="font-medium text-red-600">{data.disabledUsers}</span>
          </div>
        </div>
      </Card>
      <Card className="p-5">
        <h3 className="text-sm font-semibold text-tdop-navy mb-3">Session Management Notes</h3>
