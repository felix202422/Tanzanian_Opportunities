...
const SuperAdminNotificationConfigPage: React.FC = () => {
  ...
  const loadConfigs = async () => {
    try {
      const data = await superAdminApi.getNotificationConfig();
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await superAdminApi.setNotificationConfig(newKey, newValue, newDesc || undefined);
  ...
  const handleToggle = async (config: ConfigEntry) => {
    ...
    try {
      await superAdminApi.setNotificationConfig(config.key, newVal, config.description);
      await loadConfigs();
    } catch { /* ignore */ }
  };
            ...
            <tbody className="divide-y">
              {configs.map((config) => (
                <tr key={config.key} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-3"><Badge variant={config.value === 'true' ? 'secondary' : config.value === 'false' ? 'danger' : 'outline'}>{config.value}</Badge></td>
...
export default SuperAdminNotificationConfigPage;
