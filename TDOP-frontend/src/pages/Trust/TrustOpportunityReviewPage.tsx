...
import {
  ...
  Search, Briefcase, Clock, FileText
} from 'lucide-react';
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      {selectedItem ? (
        <div className="space-y-6">
          <button onClick={() => { setSelectedItem(null); setSearchParams({}); }} className="flex items-center gap-2 text-sm text-tdop-primary hover:underline">
          ...
          <Card className="p-6">
            <div className="flex items-start justify-between">
              ...
              <Badge variant={selectedItem.status === 'PENDING_REVIEW' || selectedItem.status === 'UNDER_REVIEW' ? 'danger' : 'primary'}>
                {selectedItem.status}
              </Badge>
            </div>
            ...
            {paginated.length === 0 ? (
              ...
              <div className="divide-y">
                {paginated.map(item => (
                  <button
                    key={item.id}
                    onClick={() => { setSelectedItem(item); setSearchParams({ review: String(item.id) }); }}
                    className="w-full flex items-center gap-4 p-4 hover:bg-gray-50 transition-colors text-left"
                  >
                    <div className="p-2 bg-blue-50 rounded-lg"><Briefcase className="w-5 h-5 text-tdop-primary" /></div>
