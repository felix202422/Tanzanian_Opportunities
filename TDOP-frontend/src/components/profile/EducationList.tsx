...
export const EducationList: React.FC<EducationListProps> = ({ education, onAdd, onRemove, className = '' }) => {
  ...
  return (
    <Card className={className}>
      ...
      {showAdd && onAdd && (
        <div className="flex gap-2 mb-4 p-3 bg-gray-50 bg-gray-50 rounded-lg animate-slide-up">
          <input
            type="text"
      ...
      {education.length === 0 ? (
        <p className="text-center text-gray-500 py-4">{t('profile.noEducation')}</p>
      ) : (
        <div className="space-y-3">
          {education.map(edu => (
            <div key={edu.id} className="flex items-start justify-between p-3 bg-gray-50 bg-gray-50 rounded-lg">
              ...
              {onRemove && (
                <button onClick={() => onRemove(edu.id)} className="p-1 rounded hover:bg-red-100 opacity-70 hover:opacity-100 transition-opacity">
