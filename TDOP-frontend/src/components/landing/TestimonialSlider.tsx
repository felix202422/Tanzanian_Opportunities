  ...
  return (
    <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
      ...
      <div className="relative max-w-3xl mx-auto">
        <div className="rounded-3xl border border-gray-100 bg-gray-50 p-8 sm:p-10 text-center animate-fade-in">
          <Quote className="w-10 h-10 mx-auto text-tdop-accent" />
          <p className="mt-6 text-lg sm:text-xl text-gray-700 leading-relaxed">"{item.quote}"</p>
          <div className="mt-8 flex items-center justify-center gap-4">
            <div className="w-14 h-14 rounded-full bg-tdop-primary/10 flex items-center justify-center text-tdop-primary font-bold text-xl">
              {item.name.charAt(0)}
            </div>
            <div className="text-left">
              <p className="font-display font-semibold text-tdop-navy">{item.name}</p>
          ...
          <div className="mt-6 flex justify-center gap-2">
            {testimonials.map((_, i) => (
              <button
                ...
                className={`w-2.5 h-2.5 rounded-full transition-colors ${i === index ? 'bg-tdop-accent w-6' : 'bg-gray-300 hover:bg-gray-400'}`}
              />
            ))}
          </div>
        )}
