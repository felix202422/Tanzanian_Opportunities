...
import { avatarFor } from './mockData';
...
const TestimonialSlider: React.FC = () => {
  ...
  const testimonials = Array.from({ length: total }, (_, i) => ({
    name: t(`landing.testimonial${i + 1}Name`),
    role: t(`landing.testimonial${i + 1}Role`),
    quote: t(`landing.testimonial${i + 1}Quote`),
    avatar: avatarFor(t(`landing.testimonial${i + 1}Name`)),
  }));
      ...
      <div className="relative max-w-3xl mx-auto">
        <div className="rounded-3xl border border-gray-100 bg-tdop-light p-8 sm:p-10 text-center animate-fade-in">
          ...
          <div className="mt-8 flex items-center justify-center gap-4">
            <img src={item.avatar} alt={item.name} className="w-14 h-14 rounded-full ring-4 ring-tdop-gold/40 object-cover" />
