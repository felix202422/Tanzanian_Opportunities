...
export const Card: React.FC<CardProps> = ({ children, className = '', hover = false, padding = true, onClick }) => {
  ...
  return (
    <div
      className={`bg-white rounded-xl border border-gray-200 shadow-card ${padding ? 'p-6' : ''} ${hover || isClickable ? 'hover:shadow-md hover:border-tdop-primary/30 cursor-pointer transition-all duration-200' : ''} ${className}`}
      onClick={onClick}
      {...(isClickable ? { role: 'button', tabIndex: 0, onKeyDown: (e: React.KeyboardEvent) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); onClick?.(); } } } : {})}
    >
      {children}
    </div>
  );
