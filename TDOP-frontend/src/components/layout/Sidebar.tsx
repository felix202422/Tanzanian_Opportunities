...
const Sidebar: React.FC<SidebarProps> = ({ className = '' }) => {
 ...
 return (
 <aside
 className={`${collapsed ? 'w-20' : 'w-64'} bg-white border-r border-gray-200 text-gray-800 hidden lg:flex flex-col min-h-[calc(100vh-4rem)] transition-all duration-300 ${className}`}
 >
 <div className="p-3">
 <button
 onClick={() => setCollapsed(c => !c)}
 aria-label={collapsed ? 'Expand sidebar' : 'Collapse sidebar'}
 className="ml-auto flex items-center justify-center w-8 h-8 rounded-lg bg-gray-100 hover:bg-gray-200 text-gray-600 transition-colors"
 >
 ...
 : 'text-gray-600 hover:bg-gray-100 hover:text-tdop-primary'
 ...
 <p className="text-xs text-gray-600">Tanzania Digital Opportunity Platform</p>
 </div>
 </aside>
 );
};
