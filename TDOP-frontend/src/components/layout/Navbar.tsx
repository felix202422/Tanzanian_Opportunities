...
const Navbar: React.FC = () => {
 ...
 : 'text-gray-600 hover:text-tdop-primary hover:bg-gray-100'
 }`}
 >
 {link.label}
 </Link>
 ...
 <div className="hidden sm:flex items-center gap-1 rounded-full border border-gray-200 p-0.5" aria-label="Language toggle">
 <Languages className="w-4 h-4 text-gray-400 ml-2" />
 <button
 onClick={() => setLanguage('en')}
 className={`px-2.5 py-1 rounded-full text-xs font-semibold transition-colors ${
 currentLang === 'en' ? 'bg-tdop-primary text-white' : 'text-gray-500 hover:text-tdop-primary'
 }`}
 >
 EN
 </button>
 ...
 className={`px-2.5 py-1 rounded-full text-xs font-semibold transition-colors ${
 currentLang === 'sw' ? 'bg-tdop-primary text-white' : 'text-gray-500 hover:text-tdop-primary'
 }`}
 >
 SW
 </button>
...
{isAuthenticated && (
 ...
 {showUserMenu && (
 <div className="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-lg border border-gray-200 py-2 z-50 animate-slide-down">
 ...
 {user?.role !== 'seeker' && (
 <Link to="/my-jobs" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">
 {t('nav.myJobs')}
 </Link>
 )}
 <button onClick={() => logout()} className="block w-full px-4 py-2 text-sm text-red-600 hover:bg-red-50 text-left">
 {t('nav.logout')}
 </button>
 </div>
 )}
